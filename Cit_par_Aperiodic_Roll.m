...
FU_AperiodicRoll = FU(t_AperiodicRoll);     % [kg]
hp0    = hp(t_AperiodicRoll);               % pressure altitude in the stationary flight condition [m]
V0     = TAS(t_AperiodicRoll);              % true airspeed in the stationary flight condition [m/sec]
alpha0 = deg2rad(AOA(t_AperiodicRoll));     % angle of attack in the stationary flight condition [rad]
th0    = deg2rad(Pitch(t_AperiodicRoll));   % pitch angle in the stationary flight condition [rad]
m      = Wi - FU_AperiodicRoll;             % [kg]
...
CL = CLa*alpha0^2;              % Lift coefficient [ ]
