...
FU_Phugoid = FU(t_Phugoid);           % [kg]
hp0    = hp(t_Phugoid);               % pressure altitude in the stationary flight condition [m]
V0     = TAS(t_Phugoid);              % true airspeed in the stationary flight condition [m/sec]
alpha0 = deg2rad(AOA(t_Phugoid));     % angle of attack in the stationary flight condition [rad]
th0    = deg2rad(Pitch(t_Phugoid));   % pitch angle in the stationary flight condition [rad]
m      = Wi - FU_Phugoid;             % [kg]
